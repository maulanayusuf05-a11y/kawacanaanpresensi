...
export default async function handler(req: any, res: any) {
  ...
  if (action === 'create_transaction') {
    ...
    // membeli Paket Guru (karena seluruh guru & admin sudah tercakup lisensi sekolah).
    // Jika masa aktif Paket Sekolah telah berakhir (expired), pengguna kembali ke mode gratis
    // dan diperkenankan untuk membeli Paket Guru atau memperpanjang Paket Sekolah.
    // ------------------------------------------------------------------------
    if (!isSchool) {
      ...
      if (school_id) {
        const { data: sch } = await db
          .from('schools')
          .select('id, name, plan, status, subscription_expires_at')
          .eq('id', school_id)
          .maybeSingle();

        if (sch) {
          const schPlan = (sch.plan || '').toLowerCase();
          const isProPlan = schPlan === 'sekolah_pro' || schPlan === 'school_pro' || schPlan === 'school' || schPlan === 'sekolah';
      ...
      if (!activeSchoolPro && b.user_id) {
        ...
        if (prof?.school_id) {
          ...
          const profPlan = (prof.subscription_plan || '').toLowerCase();
          const isProPlan = profPlan === 'sekolah_pro' || profPlan === 'school_pro';
  ...
  // --------------------------------------------------------------------------
  // 5. SETTLE DIRECT PAYMENT (MANUAL QRIS RECORDING)
  // --------------------------------------------------------------------------
  if (action === 'settle') {
    const token = (req.headers.authorization || '').replace(/^Bearer\s+/i, '').trim();
    if (!token) return json(res, 401, { error: 'Unauthorized' });
    const { data: auth, error: authError } = await db.auth.getUser(token);
    if (authError || !auth.user) return json(res, 401, { error: 'Invalid session' });
    const { data: profile } = await db.from('profiles').select('role,school_id').eq('id', auth.user.id).single();
    ...
    if (!t?.invoiceNo) return json(res, 400, { error: 'Invoice tidak valid.' });
    const { data, error } = await db.from('payments').upsert({
      invoice_no: t.invoiceNo,
      school_id: profile.role === 'SUPER_ADMIN' ? (t.schoolId || null) : profile.school_id,
      ...
      email: t.email || null,
      qris_nmid: t.qrisNmid,
      created_at: t.createdAt,
      expires_at: t.expiresAt,
      paid_at: t.paidAt || new Date().toISOString(),
