...
export const HowItWorksSection: React.FC<HowItWorksSectionProps> = ({ lang, onOpenRegister }) => {
  ...
  return (
    <section className="py-16 sm:py-20 lg:py-24 bg-slate-50 text-slate-900 relative border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-left max-w-3xl space-y-4 mb-12 sm:mb-16">
          ...
          <h2 className="text-2xl sm:text-4xl lg:text-5xl font-black text-slate-900 tracking-tight uppercase leading-[1.05]">
            {lang === 'ID' ? (
              <>
                MULAI DALAM<br />
                <span className="text-indigo-600">3 LANGKAH MUDAH</span>
              </>
            ) : (
              <>
                GET STARTED IN<br />
          ...
          {steps.map((item, idx) => {
            ...
            return (
              <div 
                ...
                className="bg-white border border-slate-200 rounded-xl p-8 shadow-2xs hover:border-indigo-400 hover:shadow-md transition-all relative flex flex-col justify-between"
              >
                <div>
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest text-indigo-600 mb-1 block font-mono">
                    {item.tag}
                  </span>
                ...
                <div className="mt-8 pt-4 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-slate-400 uppercase tracking-wider">
                  ...
                  {idx < 2 && (
                    <span className="text-indigo-600 hidden md:inline">
                      {lang === 'ID' ? 'Lanjut →' : 'Next →'}
                    </span>
                  )}
                </div>
        ...
        {/* Callout bar */}
        <div className="mt-12 p-6 bg-white border border-slate-200 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4 shadow-2xs">
          ...
          <button
            ...
            className="px-6 py-3 bg-slate-900 hover:bg-indigo-600 text-white text-xs font-bold uppercase tracking-widest transition-all rounded shrink-0 cursor-pointer shadow-sm flex items-center gap-2"
          >
