...
export const PricingSection: React.FC<PricingSectionProps> = ({ onOpenRegister, lang, customPackagesConfig }) => {
  ...
  return (
    <section id="harga" className="py-12 sm:py-20 lg:py-24 bg-slate-50 text-slate-900 relative border-b border-blue-100 antialiased">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-3 sm:space-y-4 mb-8 sm:mb-12">
          ...
          <div className="pt-2 sm:pt-4 flex items-center justify-center">
            <div className="inline-flex items-center p-1 sm:p-1.5 bg-white border border-slate-200 rounded-xl sm:rounded-2xl shadow-xs">
              <button
                type="button"
                onClick={() => setBillingCycle('monthly')}
                className={`px-3.5 sm:px-5 py-1.5 sm:py-2 rounded-lg sm:rounded-xl text-xs sm:text-sm font-extrabold transition-all cursor-pointer ${
              ...
              <button
                ...
                className={`px-3.5 sm:px-5 py-1.5 sm:py-2 rounded-lg sm:rounded-xl text-xs sm:text-sm font-extrabold transition-all flex items-center gap-1.5 sm:gap-2 cursor-pointer ${
                  billingCycle === 'yearly'
                    ? 'bg-[#0B2F64] text-white shadow-sm'
                    : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                ...
                <span className="px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-black bg-emerald-500 text-white animate-pulse">
        ...
        <div className="flex md:grid md:grid-cols-2 lg:grid-cols-4 overflow-x-auto md:overflow-x-visible snap-x snap-mandatory gap-2.5 sm:gap-5 lg:gap-6 items-stretch pb-2 md:pb-0 -mx-3 px-3 sm:mx-0 sm:px-0">
          {plans.map((plan) => {
            ...
            return (
              <div
                ...
                className={`w-[76vw] min-[360px]:w-[72vw] xs:w-[68vw] sm:w-auto shrink-0 md:shrink snap-center rounded-2xl sm:rounded-3xl p-3 xs:p-4 sm:p-6 lg:p-7 flex flex-col justify-between transition-all relative ${
                  plan.highlight
                    ? 'bg-gradient-to-b from-[#0B2F64] to-[#071F42] text-white border-2 border-blue-600 shadow-xl lg:-translate-y-2.5 ring-4 ring-blue-500/10'
                    : 'bg-white text-slate-900 border border-slate-200 shadow-xs hover:shadow-md'
                }`}
              >
                {/* Badge Status / Rekomendasi */}
                {plan.badge && (
                  <div className={`absolute -top-3 left-4 sm:left-6 px-2.5 sm:px-3.5 py-0.5 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-wider shadow-sm flex items-center gap-1 sm:gap-1.5 ${
                    plan.highlight
                ...
                <div>
                  ...
                  <div className={`flex items-center gap-1.5 text-[9.5px] sm:text-[11px] font-bold uppercase tracking-wider mb-1 sm:mb-2 ${
                  ...
                  {/* Plan Name */}
                  <h3 className={`text-sm sm:text-2xl font-black uppercase tracking-tight mb-0.5 sm:mb-2 ${
                  ...
                  {/* Tagline */}
                  <p className={`text-[11px] sm:text-xs mb-2 sm:mb-5 min-h-0 sm:min-h-[40px] leading-snug sm:leading-relaxed ${
                  ...
                  {/* Price Block */}
                  <div className={`mb-2 pb-2 sm:mb-6 sm:pb-6 border-b ${
                    plan.highlight ? 'border-blue-800/80' : 'border-slate-100'
                  }`}>
                    <div className="flex items-baseline gap-1 flex-wrap">
                      <span className={`text-lg xs:text-xl sm:text-3xl lg:text-4xl font-black tracking-tight ${
                    ...
                    {plan.savingsBadge && (
                      <div className="mt-1">
                        <span className={`inline-block px-1.5 sm:px-2 py-0.5 rounded text-[8.5px] sm:text-[10px] font-black uppercase tracking-wider ${
                          plan.highlight
                    ...
                    <div className={`mt-1 sm:mt-2.5 text-[8.5px] sm:text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5 ${
                      plan.highlight ? 'text-blue-200' : 'text-slate-500'
                    }`}>
                      <QrCode className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0 text-blue-500" />
                      <span>{plan.paymentNote}</span>
                    </div>
                  </div>

                  {/* Features List */}
                  <div className="space-y-1 sm:space-y-2.5 mb-2.5 sm:mb-6 lg:mb-8">
                    ...
                    {plan.features.map((feat: string, i: number) => (
                      <div key={i} className={`flex items-start gap-1.5 sm:gap-2.5 leading-snug ${
                        plan.highlight ? 'text-blue-50' : 'text-slate-700'
                      }`}>
                        <div className={`w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${
                        ...
                        <span className="text-[10.5px] sm:text-xs">{feat}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <button
                  ...
                  className={`w-full py-2 sm:py-3.5 px-3 sm:px-4 text-[11px] sm:text-xs font-black uppercase tracking-wider transition-all cursor-pointer rounded-xl flex items-center justify-center gap-2 active:scale-95 ${plan.ctaStyle}`}
