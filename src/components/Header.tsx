          ...
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
            {currentUser ? (
              <div className="flex items-center gap-1.5 sm:gap-2.5">
                ...
                {/* Compact Workspace Indicator & Switcher Button */}
                {currentUser.role !== 'SUPER_ADMIN' && currentUser.role !== 'SISWA' && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      } else if (isCurrentlyPersonal) {
                        setIsJoinSchoolModalOpen(true);
                      }
                    }}
                    disabled={!canSwitchWorkspace && !isCurrentlyPersonal}
                    title={
                      canSwitchWorkspace
                        ? `Klik untuk beralih antara Ruang Kerja Sekolah dan Individu (Saat ini: ${isCurrentlyPersonal ? 'Ruang Kerja Individu' : 'Ruang Kerja Sekolah'})`
                        : `Ruang Kerja: ${isCurrentlyPersonal ? 'Ruang Kerja Individu' : (schoolProfile.namaSekolah || 'Ruang Kerja Sekolah')}`
                    }
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full border text-xs font-bold transition-all select-none ${
                      isCurrentlyPersonal
                        ? 'bg-emerald-50 hover:bg-emerald-100 border-emerald-200 text-emerald-800'
                        : 'bg-indigo-50 hover:bg-indigo-100 border-indigo-200 text-indigo-800'
                    } ${canSwitchWorkspace ? 'cursor-pointer active:scale-95 shadow-2xs' : 'cursor-default'}`}
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">
                      {isCurrentlyPersonal ? 'Ruang Individu' : 'Ruang Sekolah'}
                    </span>
                    ...
                    {canSwitchWorkspace && (
                      <span className={`text-[9px] px-1 py-0.5 rounded font-black uppercase ${
                        isCurrentlyPersonal ? 'bg-emerald-200/80 text-emerald-900' : 'bg-indigo-200/80 text-indigo-900'
                      }`}>
                        Ganti
                      </span>
                    )}
